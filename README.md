...
└── data/
    ...
    └── amenities_poi.json     ~7,700 OpenStreetMap POIs by category (for map overlays)
```

`data.js` is a plain concatenation of the seven files in `/data` into JS `const`
...
for name, path in [
    ...
    ('AMENITIES_POI', 'data/amenities_poi.json'),
]:
